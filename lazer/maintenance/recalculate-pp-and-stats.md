...
本节介绍了如何使用 `tools/recalculate.py` 工具来重新计算 pp、排行榜、难度星级等数据。
...
你可以使用子命令 `performance` 来重新计算 pp，并支持使用一些过滤器。请使用 `-h` 选项查看可用的过滤器。
...
如果你更改了有关谱面或统计信息的设置，可以使用子命令 `leaderboard` 进行重新计算。同样可以使用 `-h` 选项查看过滤器。
...
如果你更换或更新了表现分计算器，可能需要重新计算所有谱面的难度星级（Star Ratings）。使用 `rating` 子命令。使用 `-h` 选项查看过滤器。
...
我们提供了一个快捷子命令 `all` 来一次性重新计算星级、pp 和排行榜。执行顺序为：难度星级 (rating) -> pp (performance) -> 排行榜 (leaderboard)。
